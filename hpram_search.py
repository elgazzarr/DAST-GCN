...
from data import prepare_data, prepare_data_age
import os
import shutil
import ray
...
def train(args):
    ...
    train_loader, val_loader, test_loader, adj_mx, seq_len, num_nodes = prepare_data_age(n=args.n_samples, dim=args.in_dim, bs=args.batch_size)
    engine = trainer(in_dim=args.in_dim, kernel=args.kernel, pool=args.pool, res=args.res, num_nodes=num_nodes, filters=args.n_filters , dropout=args.dropout, lrate=args.lr, wdecay=args.weight_decay, device=args.device, supports=None, blocks=args.blocks)
...
def run_search(config, checkpoint_dir=None):
    ...
    tune.report(Auc=test_results['auc'],
                Acc=test_results['acc'],
                Sen=test_results['prec'],
                Spc=test_results['recall'],
                loss=min_val)
...
if __name__ == "__main__":
    ...
    params = {
        ...
        'filters': tune.choice([4, 6, 8, 16, 32]),
    ...
    time.sleep(30)
    analysis = tune.run(run_search, name='DAST-GCN_age',
                        config=params,
                        num_samples=200,
                        resources_per_trial={"cpu": 10, "gpu": 1})
    print(analysis.get_best_config(metric='Auc'))
    print(analysis.get_best_trial(metric='Auc'))
